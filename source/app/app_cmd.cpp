...
//	printf("PE9--AC connect: %d, is AC connect:%d\n", ssz_gpio_is_high(SYS_PWR_COM_PORT, SYS_PWR_ADAPTER_INSERT_PIN), sys_power_is_
...
int app_cmd_io_install(const char* cmd, char* params[], int param_size)
{
	pill_box_install_sensor_left_pwr_enable();
	pill_box_install_sensor_right_pwr_enable();
	printf("pill box is install:  ");

	if( ( pill_box_install_left_detect() == 1 ) || ( pill_box_install_right_detect() == 1 ) )
	{
		printf("Yes.\n ");
	}
	else
...
int app_cmd_io_encoder(const char* cmd, char* params[], int param_size)
{
	//需要辅MCU使能电源
	printf("optical coupler(need slaver enable power): %d\n", ssz_gpio_is_high(SZDJ_SUDU_GPIO_Port, SZDJ_SUDU_Pin));
}
...
int app_cmd_ad(const char* cmd, char* params[], int param_size)
{
	...
	printf("battery real voltage:%d mv \n",voltage);
...
int app_cmd_ad_pressure(const char* cmd, char* params[], int param_size)
{
	pressure_and_bubble_sensor_pwr_enable();
...
static int app_cmd_STF_position(const char* cmd, char* params[], int param_size)
{
	...
	printf("STF sensor position is : ");
	if( drv_three_valve_is_pos_a_detected() )
	{
		printf("A\n");
	}
	else if(drv_three_valve_is_pos_b_detected())
	{
		printf("B\n");
	}
	else
	{
		printf("not sure position\n");
	}
...
const static AppCmdInfo g_app_cmd_info[] =
{
	...
	{"io_install", app_cmd_io_install, "show install sensor value"},
	{"io_encoder", app_cmd_io_encoder, "show install sensor value"},	
	...
	{"ad_pressure", app_cmd_ad_pressure, "show pressure adc value" },
	...
	{"io_stf_encoder", app_cmd_STF_position, "check valve position, return value is A or B"},	
